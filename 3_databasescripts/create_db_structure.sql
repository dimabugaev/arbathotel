...
CREATE TABLE operate.card_assignments (
    id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,

    card_source_id int NOT NULL,
    report_source_id int NOT NULL,

    start_date date NOT NULL,
    end_date date,

    CONSTRAINT fk_source_cards FOREIGN KEY ( card_source_id ) REFERENCES operate.sources  ( id ),
    CONSTRAINT fk_source_reports FOREIGN KEY ( report_source_id ) REFERENCES operate.sources  ( id )
);

CREATE UNIQUE INDEX ux_card_assignments_active
ON operate.card_assignments (card_source_id)
WHERE end_date IS NULL;

--not finished
CREATE TABLE operate.companies
(
	 id	int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
	 company_bank_statement_name varchar NOT null,
...
drop table if exists operate.contragents;
drop table if exists operate.budget_items;
CREATE TABLE operate.budget_items
(
	 id	int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
	 perfix varchar NOT null,
...
CREATE TABLE operate.contragents
(
	 id	int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
	 contragent_name varchar NOT null,
	 inner_name varchar NOT null,
	 inn varchar NOT null,
	 account_number varchar,
	 income_budget_item_id int,
	 outcome_budget_item_id int,
	 hotel_id int,
	 source_id int UNIQUE,
	 is_hidden bool default false,
	 
	 CONSTRAINT fk_income_budget_item_contragents FOREIGN KEY ( income_budget_item_id ) REFERENCES operate.budget_items  ( id ),
	 CONSTRAINT fk_outcome_budget_item_contragents FOREIGN KEY ( outcome_budget_item_id ) REFERENCES operate.budget_items  ( id ),
	 CONSTRAINT fk_hotel_contragents FOREIGN KEY ( hotel_id ) REFERENCES operate.hotels  ( id ),
	 CONSTRAINT fk_source_contragents FOREIGN KEY ( source_id ) REFERENCES operate.sources  ( id )
);
...
--	 source_id int,
     ...
     CONSTRAINT fk_contragents_items FOREIGN KEY ( contragent_id ) REFERENCES operate.contragents ( id )
--	 CONSTRAINT fk_hotels_items FOREIGN KEY ( hotel_id ) REFERENCES operate.hotels ( id ),
--	 CONSTRAINT fk_empl_items FOREIGN KEY ( empl_id ) REFERENCES operate.employees ( id ),
--	 CONSTRAINT fk_source_items FOREIGN KEY ( source_id ) REFERENCES operate.sources  ( id )
);
...
create index idx_report_strings_auto_outer_business_id
on operate.report_strings (outer_row_business_id)
where outer_row_business_id is not null;
